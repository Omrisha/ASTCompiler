...
#define APPLY_OP_ON_NUM_NODE(lhs, rhs, opcode) (lhs->_type == _INT \
                                                ? new NumNode(lhs->_u.ival opcode rhs->_u.ival) \
                                                : new NumNode(lhs->_u.fval opcode rhs->_u.fval))
#define GET_VALUE(node) (node->_type == _INT ? node->_u.ival : node->_u.fval)
...
public:
    ...
    Exp* getLeft() {
        return getNode(_left);
    }

    Exp* getRight() {
        return getNode(_right);
    }

    Exp* getNode(Exp* exp) {
        BinaryOp* binary_op = dynamic_cast<BinaryOp *>(exp);
        if (!binary_op) {
            return exp;
        }

        Exp* left = binary_op->getLeft();
        Exp* right = binary_op->getRight();

        if (!left || !right) {
            return nullptr;
        }

        NumNode* left_result = dynamic_cast<NumNode *>(left);
        NumNode* right_result = dynamic_cast<NumNode *>(right);

        if (!left_result || !right_result) {
            return exp;
        }

        return BinaryOp::applyAlgebricOperatorOnNodes(left_result, right_result, binary_op->_op);
    }

    static NumNode* applyAlgebricOperatorOnNodes(NumNode* lhs, NumNode* rhs, enum op opcode) {
        if (!lhs || !rhs)
            return nullptr;
        switch (opcode) {
            case PLUS:
                return APPLY_OP_ON_NUM_NODE(lhs, rhs, +);
            case MINUS:
                return APPLY_OP_ON_NUM_NODE(lhs, rhs, -);
            case MUL:
                return APPLY_OP_ON_NUM_NODE(lhs, rhs, *);
            case DIV:
                return APPLY_OP_ON_NUM_NODE(lhs, rhs, /);
            case XOR_OP:
                if (lhs->_type == _FLOAT || rhs->_type == _FLOAT) {
                    return nullptr;
                }

                return new NumNode(lhs->_u.ival ^ rhs->_u.ival);
            default:
                return nullptr;
        }
        assert(0 && "You have reached an invalid state.");
    }

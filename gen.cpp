...
Object BinaryOp::genExp () {
    ...
    NumNode* left_num_node = dynamic_cast<NumNode *>(left);
    NumNode* right_num_node = dynamic_cast<NumNode *>(right);
    if (left && right) {
        NumNode* result = BinaryOp::applyAlgebricOperatorOnNodes(left_num_node, right_num_node, _op);
        if (result) {
            return result->genExp();
        }
    }
    ...
    const char *the_op = opName(_op, _type);//_left->_type == _FLOAT || _right->_type == _FLOAT ? _FLOAT : _INT);
...
{
	if (_exp->_type != _INT)
		errorMsg("expression is not Integer.\n");

	Object t1 = _exp->genExp();
	for (Case *stmt = _caselist; stmt != NULL; stmt = stmt->_next)
	{
		stmt->_label = newlabel();
		emit("if t1 == %d goto label %d\n", stmt->_number, stmt->_label);
	}
	int defaultLabel = newlabel();
	emit("goto label %d\n", defaultLabel);
	int haltLabel = newlabel();

	for (Case *stmt = _caselist; stmt != NULL; stmt = stmt->_next)
	{
		emitlabel(stmt->_label);
		stmt->_stmt->genStmt();
		
		if (stmt->_label < defaultLabel)
			emit("goto label %d\n", haltLabel);
	}
